...
func (s *Selector) Acquire(ctx context.Context, provider account.Provider, modelRouteID uint64, upstreamModel, quotaMode, affinityKey string, excluded map[uint64]bool, allowQuotaProbe bool) (*accountLease, error) {
	return s.AcquireEligible(ctx, provider, modelRouteID, upstreamModel, quotaMode, affinityKey, excluded, allowQuotaProbe, nil)
}

// AcquireEligible 在通用账号健康条件之外应用调用方的能力过滤。
// 过滤仅影响本次选择，不修改账号健康、额度或模型冷却状态。
func (s *Selector) AcquireEligible(ctx context.Context, provider account.Provider, modelRouteID uint64, upstreamModel, quotaMode, affinityKey string, excluded map[uint64]bool, allowQuotaProbe bool, eligible func(account.Credential) bool) (*accountLease, error) {
	now := time.Now().UTC()
