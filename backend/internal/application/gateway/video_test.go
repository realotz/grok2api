...
func TestVideoEditRouteUsesResolvedUpstreamInsteadOfPublicName(t *testing.T) {
	...
	compatible, err := routesForVideoOperation(routes, provider.VideoOperationEdit, 0)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := routesForVideoOperation(routes[:1], provider.VideoOperationExtend, 0); !errors.Is(err, ErrVideoOperationUnsupported) {
		t.Fatalf("unsupported route error = %v", err)
	}
}

func TestVideoExtensionRouteUsesWeb15WhenStartTimeProvided(t *testing.T) {
	routes := []model.Route{
		{ID: 1, Provider: account.ProviderConsole, UpstreamModel: "grok-imagine-video"},
		{ID: 2, Provider: account.ProviderWeb, UpstreamModel: "grok-imagine-video-1.5"},
	}
	compatible, err := routesForVideoOperation(routes, provider.VideoOperationExtend, 16.031666)
	if err != nil {
		t.Fatal(err)
	}
	if len(compatible) != 1 || compatible[0].ID != 2 {
	...
	if err := validateVideoExtensionRoute(provider.VideoOperationExtend, compatible[0], 10, 16.031666); err != nil {
		t.Fatal(err)
	}
	if err := validateVideoExtensionRoute(provider.VideoOperationExtend, compatible[0], 5, 6.041667); !errors.Is(err, ErrVideoOperationUnsupported) {
...
func TestVideoRouteParametersRejectConsoleReferenceLimits(t *testing.T) {
	if err := checkVideoRouteParameters(provider.VideoOperationGenerate, account.ProviderConsole, "grok-imagine-video-1.5", "720p", 15, 0, 6); !errors.Is(err, ErrVideoParameterInvalid) {
		t.Fatalf("15 references error = %v", err)
	}
	if err := checkVideoRouteParameters(provider.VideoOperationGenerate, account.ProviderConsole, "grok-imagine-video-1.5", "720p", 14, 0, 6); err != nil {
		t.Fatalf("14 references error = %v", err)
	}
	// 旧模型仍保持 Console 的 7 张限制。
	if err := checkVideoRouteParameters(provider.VideoOperationGenerate, account.ProviderConsole, "grok-imagine-video", "720p", 8, 0, 6); !errors.Is(err, ErrVideoParameterInvalid) {
		t.Fatalf("legacy 8 references error = %v", err)
	}
	...
	// Build 使用相同的 1.5 上游模型名，也支持 14 张。
	if err := checkVideoRouteParameters(provider.VideoOperationGenerate, account.ProviderBuild, "grok-imagine-video-1.5", "720p", 14, 0, 15); err != nil {
...
func TestRoutesForVideoParametersKeepsCompatibleSameNameProviders(t *testing.T) {
	routes := []model.Route{
		{ID: 1, PublicID: "shared-video", Provider: account.ProviderConsole, UpstreamModel: "grok-imagine-video"},
...
func TestCreateVideoAppliesRouteConstraintsAfterKeyEligibilityAndBeforeInputIO(t *testing.T) {
	...
	references := make([]string, media.MaxModelReferenceImages+1)
...
func TestVideo1080pValidationUsesResolvedUpstreamModel(t *testing.T) {
	...
	if err := checkVideoRouteParameters(provider.VideoOperationGenerate, account.ProviderWeb, "grok-imagine-video-1.5", "720p", 15, 0, 6); !errors.Is(err, ErrVideoOperationUnsupported) {
...
func TestEncodeDecodeVideoInputPreservesOperationAndReferenceAudio(t *testing.T) {
	...
	encoded, err = encodeVideoInputFull(provider.VideoOperationExtend, "", nil, nil, media.InputReference("source-video"), 16.031666)
	if err != nil {
		t.Fatal(err)
	}
	...
	if startTime := decodeVideoExtensionStartTime(encoded); startTime != 16.031666 {
...
func (s *videoAssetStoreStub) OpenImage(_ context.Context, id string) (media.Asset, io.ReadCloser, error) {
	if s.imageAsset.ID == "" || id != s.imageAsset.ID {
		return media.Asset{}, nil, errors.New("image not found")
	}
	return s.imageAsset, io.NopCloser(bytes.NewReader(s.imageData)), nil
}

func (s *videoAssetStoreStub) PublicImageURL(id string) string {
	return strings.TrimSuffix(s.publicBase, "/") + "/v1/media/images/" + id
}
