...
func TestNewQuotaViewFreeUsesObservedRollingTokens(t *testing.T) {
	quota := newQuotaView(&accountdomain.Billing{IsUnifiedBillingUser: true}, 250_000, nil, "grok-4.5-build-free", false, accountdomain.WebTierAuto)
...
func TestNewQuotaViewPaidUsesMonthlyBilling(t *testing.T) {
	quota := newQuotaView(&accountdomain.Billing{MonthlyLimit: 200, Used: 50, BillingPeriodStart: "start", BillingPeriodEnd: "end"}, 900_000, nil, "", false, accountdomain.WebTierAuto)
...
func TestNewQuotaViewPaidShowsBillingProbeState(t *testing.T) {
	...
	}, "", false, accountdomain.WebTierAuto)
...
func TestNewQuotaViewUnknownWithoutBillingSnapshot(t *testing.T) {
	quota := newQuotaView(nil, 100, nil, "", false, accountdomain.WebTierAuto)
...
func TestNewQuotaViewEstimatesFreeFromObservedZeroBillingProfile(t *testing.T) {
	quota := newQuotaView(&accountdomain.Billing{PlanName: "Free", IsUnifiedBillingUser: true, TopUpMethod: "TOP_UP_METHOD_SAVED_PAYMENT_METHOD"}, 100, nil, "", false, accountdomain.WebTierAuto)
...
func TestNewQuotaViewTreatsZeroUsageSuperGrokAsPaid(t *testing.T) {
	...
	}, 0, nil, "", false, accountdomain.WebTierAuto)
...
func TestNewQuotaViewBuildSuperEntitlementOverridesFreeSignals(t *testing.T) {
	...
	quota := newQuotaView(&accountdomain.Billing{IsUnifiedBillingUser: true, TopUpMethod: "TOP_UP_METHOD_SAVED_PAYMENT_METHOD"}, 100, nil, "grok-4.5-build-free", true, accountdomain.WebTierAuto)
	...
	}, "", true, accountdomain.WebTierAuto)
	...
	quota = newQuotaView(&accountdomain.Billing{MonthlyLimit: 200, Used: 50}, 0, nil, "", true, accountdomain.WebTierAuto)
	if quota.Type != QuotaTypePaid || quota.Source != "upstreamBilling" || quota.Limit != 200 {
		t.Fatalf("paid billing must win: %#v", quota)
	}
}

func TestNewQuotaViewUsesLinkedWebTier(t *testing.T) {
	free := newQuotaView(&accountdomain.Billing{}, 100, nil, "", false, accountdomain.WebTierBasic)
	if free.Type != QuotaTypeFree || free.Source != "linkedWebTier" || free.Confidence != "observed" {
		t.Fatalf("linked Basic quota = %#v", free)
	}

	for _, tier := range []accountdomain.WebTier{accountdomain.WebTierSuper, accountdomain.WebTierHeavy} {
		paid := newQuotaView(&accountdomain.Billing{}, 0, nil, "grok-4.5-build-free", false, tier)
		if paid.Type != QuotaTypePaid || paid.Source != "linkedWebTier" || paid.Confidence != "observed" {
			t.Fatalf("linked %s quota = %#v", tier, paid)
		}
	}
}

func TestNewQuotaViewPaidSignalsOverrideLinkedWebBasic(t *testing.T) {
	paidBilling := newQuotaView(&accountdomain.Billing{MonthlyLimit: 200, Used: 50}, 0, nil, "", false, accountdomain.WebTierBasic)
	if paidBilling.Type != QuotaTypePaid || paidBilling.Source != "upstreamBilling" || paidBilling.Limit != 200 {
		t.Fatalf("paid Billing must override linked Basic: %#v", paidBilling)
	}

	entitled := newQuotaView(&accountdomain.Billing{}, 0, nil, "", true, accountdomain.WebTierBasic)
	if entitled.Type != QuotaTypePaid || entitled.Source != "buildSuperEntitlement" {
		t.Fatalf("Build entitlement must override linked Basic: %#v", entitled)
	}
}
