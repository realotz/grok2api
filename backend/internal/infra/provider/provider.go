...
// VideoFailureStage identifies where an upstream video request failed.
...
// VideoFailureError records whether retrying the request on another account is safe.
...
// NewVideoFailureError attaches lifecycle and retry semantics to a provider error.
...
// VideoFailureDetails extracts video failure semantics from an error chain.
