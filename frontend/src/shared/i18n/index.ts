    ...
    translation: {
      ...
      docs: {
        ...
        reference: {
          ...
          noteVideoReference: "参考图/参考音频走 /videos/generations。只传 1 张 reference_images 也保持参考语义，不会升格为首帧。网关最多 8 张图、3 段音频；Console 图最多 7 张。不可与 image 同用，参考模式最高 720p。prompt 使用官方 <IMAGE_n>/<AUDIO_n>（不区分大小写）。voice_id 三端一致；Web 会按 grok.com 预置音色目录（缓存 1 小时）映射为资产 UUID，自定义音频仍走 url/data。",
    ...
    translation: {
      ...
      docs: {
        ...
        reference: {
          ...
          noteVideoReference: "Reference-to-video uses /videos/generations. A single reference_images item keeps reference semantics and is not promoted to a first frame. The gateway accepts up to 8 images and 3 audio clips; Console image routes accept up to 7. Do not combine them with image. Reference mode tops out at 720p. Prompts use official <IMAGE_n>/<AUDIO_n> tags (case-insensitive). voice_id is the same on Web, Build, and Console; Web maps preset voices from the grok.com catalog (1-hour cache) to asset UUIDs, and still accepts custom url/data.",
